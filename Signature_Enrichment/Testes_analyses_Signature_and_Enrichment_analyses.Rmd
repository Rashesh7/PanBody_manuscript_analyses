...
load_VCF <- function(dataset, surveyed = NA) {
  ...
  if(dataset == "PopVars") {
    gnomadVCF <- read_vcfs_as_granges(paste0(path,"variantVCFs/PopVars/gnomad.genomes.r2.1.1.snps.filtered.singletons.vcf"), "gnomad", ref_genome)
    return(gnomadVCF)
  }
  ...
  if(dataset == "Testes") {
    ...
    testesIndivVCF <- read_vcfs_as_granges(testesIndivFiles, testesIndivSamples, ref_genome)
  ...
  if(dataset == "Colon") {
    ...
    colonIndivVCF <- read_vcfs_as_granges(colonIndivFiles, colonIndivSamples, ref_genome)
    return(colonIndivVCF)
  }
  #Trios from DeCode
  if(dataset == "Trios") {
    triosVCF <- read_vcfs_as_granges(paste0(path, "variantVCFs/Trios/trios_decode.vcf"), "Trio_data", ref_genome)
    return(triosVCF)
  }
  ...
  if(dataset == "PanBody") {
    ...
    panBodyVCF <- read_vcfs_as_granges(panBodyFiles, panBodySamples, ref_genome)
    return(panBodyVCF)
  }
}
...
write_tsv(strand_bias, file = paste0(plotsPath, 'strand_bias.tsv'))
...
write_tsv(mainRepDist, file = paste0(plotsPath, 'repTimingIndiv.tsv'))
...
write_tsv(repTimingAdjusted, file = paste0(plotsPath, 'repTimingSummarized.tsv'))

repTimingCIs <- calculateCIs(repTimingAdjusted, VCFs, regions_list, tissues, n_sim = n_sims)
write_tsv(repTimingCIs, file = paste0(plotsPath, 'repTimingCIs.tsv'))
```
...
write_tsv(exonicDist, file = paste0(plotsPath, 'exonicIndiv.tsv'))

exonicAdjusted <- exonicDist %>% 
  group_by(region, tissueType) %>% 
  dplyr::summarise(across(where(is.numeric),sum), .groups = "drop") %>% 
  mutate(by = tissueType)
write_tsv(exonicAdjusted, file = paste0(plotsPath, 'exonicSummarized.tsv'))

exonicCIs <- calculateCIs(exonicAdjusted, VCFs, exonicRegions_list, tissues, n_sim = n_sims)
write_tsv(exonicCIs, file = paste0(plotsPath, 'exonicCIs.tsv'))
```
...
write_tsv(exprEnrichDF, file = paste0(plotsPath, 'exprSummarizedCIs.tsv'))
```

## Xia Germline expr
```{r xiaGermExpr, fig.height=4, fig.width=7}
...
ensembl_IDs <- read_tsv(paste0(path, "expression/hgncIDs2.txt"), col_types = 'ccccic', skip = 1, 
...
germCellExpr <- read_csv(paste0(path, "expression/expr_level_groups.csv"), col_types = cols(.default = "c")) %>% 
  ...
  select(group, gene_name) %>% 
  #combine 7 and 8
  mutate(group = if_else(group %in% c("Group_7", "Group_8"), "Group_7+8", group))

geneCoords0 <- gtexMedian %>% 
  select(gene, gene_name = Description) %>% 
  left_join(germCellExpr) %>%
  drop_na() %>% 
  #Join hg19 coords
  left_join(biomart, by = "gene") %>% 
  #Drop non-standard chromosomes
...
groupSizes <- geneCoords0 %>% 
  group_by(group) %>% 
  summarize(n_genes = n()) %>% 
  mutate(group = fct_relevel(group, c("Group_Unexp","Group_1","Group_2","Group_3","Group_4","Group_5","Group_6","Group_7+8"))) %>% 
  arrange(group) 


#Add same expr values for popVars + trioDNMS
geneCoords <- geneCoords0 %>% 
  bind_rows(geneCoords0 %>% mutate(tissue = "trio_DNMs")) %>% 
  bind_rows(geneCoords0 %>% mutate(tissue = "pop_vars"))

exprDataXia <- c("testis","trio_DNMs", "pop_vars")
tissuesWithExprDataXia <- exprDataXia[exprDataXia %in% tissues]

exprEnrichXia <- tibble()
for(etissue in tissuesWithExprDataXia) {
  expr_list=list()
  for(eBin in c("Group_Unexp","Group_1","Group_2","Group_3","Group_4","Group_5","Group_6","Group_7+8")){
    exprDF <- geneCoords %>% filter(group == eBin & tissue == etissue)
    expr_gr <- GRanges(exprDF$chr, IRanges(start=exprDF$txStart,end=exprDF$txEnd), strand = exprDF$strand)
    seqlevelsStyle(expr_gr) <- "UCSC"
    expr_list[[eBin]] <- expr_gr
  }
  names(expr_list) = c("Group_Unexp","Group_1","Group_2","Group_3","Group_4","Group_5","Group_6","Group_7+8")
  enrich <- genomic_distributionCustom(VCFs[etissue == tissues], surveyed_list[etissue == tissues], expr_list, triRateTable, tissues[etissue == tissues], addSigs = useSigs, probabilityDF)
  ...
  #Add CIs
  exprEnrichCIsXia <- calculateCIs(exprEnrichAdjusted, VCFs[etissue == tissues], expr_list, tissues[etissue == tissues], n_sim = n_sims)
  #Add tissue to master DF
  exprEnrichXia <- bind_rows(exprEnrichXia, exprEnrichCIsXia)
}

write_tsv(exprEnrichXia, file = paste0(plotsPath, 'exprSummarizedXia.tsv'))

exprEnrichXia <- read_tsv(paste0(plotsPath, 'exprSummarizedXiaPopVars.tsv'))
pExprXia <- exprEnrichXia %>% 
  mutate(by = fct_relevel(by, c("trio_DNMs", "pop_vars", "testis"))) %>% 
  mutate(region = fct_relevel(region, c("Group_Unexp","Group_1","Group_2","Group_3","Group_4","Group_5","Group_6","Group_7+8"))) %>% 
...
png(paste0(plotsPath, 'germExprXia.png'), height=4, width=6, units = "in", res = 300)
pExprXia 
dev.off()

pdf(paste0(plotsPath, 'germExprXia.pdf'), height=4, width=6)
pExprXia 
dev.off()
